...
-- Add resource if URL is local & relative
local function add_resource(url)
  if url:sub(1, 1) ~= '/' and
      (url:match("^%a+://") or url:match("^mailto:")) == nil then
    table.insert(resources, pandoc.MetaString(url))
  end
end
