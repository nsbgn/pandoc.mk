...
function take(pattern, str)
    local out = ""
    local str = str
    local i, j = str:find(pattern)
    if i and j then
        out = str:sub(i,j)
...
function subst(string, substitution)
...
return {
    {
        Link = function (a)
            if #a.content == 0 then
                url = a.target:gsub("^mailto:", "")
                a.content = { pandoc.Code(url) }
            elseif #a.content == 1
            and a.content[1].t == "Str"
            and a.content[1].text:gsub("%%%a","") == "" then
                a.content = { pandoc.Code(subst(a.content[1].text, dissect(a.target))) }
            end
            return a
        end,
    }
